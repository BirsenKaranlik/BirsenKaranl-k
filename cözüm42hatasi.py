function register_VIM_TUTORIAL_SECTIONS(interpreter, messager, createSection, registerSection, showCommandOneByOne, doc) {
  ...
  var introduction_section = createSection("Introduction",
    ...
    [
        ...
        "To practice what you've learned, try out the |sandbox| page. It has a context sensitive reference for commands.",
        "Now, let me introduce you to basics of Vim."
    ], defaultPost);

    var two_modes_section = createSection("Two modes, insert and command",
        defaultPre,
    [
        "Vim has two basic modes. One is |insert| mode, in which you write text as if in normal text editor.",
        "Another is |command| mode which provides you efficient ways to navigate and manipulate text.",
        "At any time, you can see in what mode you are in the status bar which is located at the top of the editor.",
        "To change between modes, there is |Esc| for command mode and |i| for insert mode",
        ...
        showCommandOneByOne(
            [
             cmd("i", function() {
               ...
               insertText("Good, now you're in insert mode. Write something and change back to command mode.");
             }),
             cmd("Esc", function() {
    ...
    var basic_movement = createSection("Basic movement: h, j, k, and l",
        defaultPre,
    [
        "In contrast to regular text editor, you use keys |h|, |j|, |k|, and |l| instead of arrow keys to move cursor.",
        ...
        showCommandOneByOne([
          "h", "h", "l", "l", "k", "h", "j",
          cmd("Enter", function() {
            insertText("Later you can get a better feel for moving in the sandbox view. Let's move on.");
          }), "Enter"],
          accepterCreator);
    });
    ...
    var times_movement = createSection("Number powered movement, e.g. 5w",
      defaultPre,
      [
          "Moving within the text is not limited to individual keys; you can combine movement keys with a |number|. For example, |3w| would do the same as if you pressed w three times."
      ],
      function() {
        interpreter.environment.setCommandMode();
    ...
    var find_occurrence = createSection("Find a character, f and F",
        ...
        [
            ...
            "You can combine f with number, so 3rd occurrence of 'q' can be found with |3fq|, que?"
        ],
        function() {
          interpreter.environment.setCommandMode();
          interpreter.interpretSequence("0");
    ...
    var matching_parentheses = createSection("Go to matching parentheses, %",
      defaultPre,
      [
        "In text that is structured with parentheses, |(| or |{| or |[|, it's possible to go to matching parentheses with |%|",
    ...
    var goto_line = createSection("Goto line, g and G",
        defaultPre,
        [
         "|gg| takes you at the beginning of the file; |G| to the end.",
    ...
    var search_match = createSection("Search, /text with n and N",
      defaultPre,
      [
        "Searching text is vital part of any text editor. In Vim, you press |/|, and give the text you are looking for.",
    ...
    var removing = createSection("Removing a character, x and X",
        defaultPre,
      [
      "|x| and |X| remove a character under cursor and left to the cursor, respectively",
      "Try pressing |x| and remove the last word."
      ], function() {
        interpreter.environment.setCommandMode();
        showCommandOneByOne([
          "x", "x", "x", "x", "x",
    ...
    var adding_line = createSection("Insert new line, o and O",
      defaultPre,
        [
            ...
            "After new line is created, editor is set to |insert| mode.",
            "Write a bit and get back to |command| mode."
        ], function() {
            interpreter.environment.setCommandMode();
    ...
    var deleting = createSection("Deleting, d",
        defaultPre,
      [
      "|d| is command for deleting",
      "You can combine it with movement, e.g. |dw| deletes the first word on the right side of the cursor",
      "It also copies the content so that you can paste it with |p| to other location (on real Vim)."
      ], function() {
        interpreter.environment.setCommandMode();
  ...
  var visual_mode = createSection("Visual mode, v",
    defaultPre,
    [
      "Besides insert and command mode, Vim has also |visual| mode.",
  ...
  var visual_block_mode = createSection("Visual block mode, ctrl-v",
    defaultPre,
    [
      "There is yet another mode: |visual block|. It makes possible to insert text on many lines at once. Let's see how with an example list.",
      "<> A smart girl",
      "<> Ulysses",
      "<> Learn and teach",
